...
class ClassificationAlgorithms:
    ...
    def k_nearest_neighbor(
        ...
        if gridsearch:
            ...
            knn = GridSearchCV(
                KNeighborsClassifier(), tuned_parameters, cv=3, scoring="roc_auc"
            )
        else:
    ...
    def decision_tree(
        ...
        if gridsearch:
            ...
            dtree = GridSearchCV(
                DecisionTreeClassifier(), tuned_parameters, cv=3, scoring="roc_auc"
            )
        else:
    ...
    def random_forest(
        ...
        if gridsearch:
            ...
            rf = GridSearchCV(
                RandomForestClassifier(), tuned_parameters, cv=3, scoring="roc_auc"
            )
        else:
    ...
    def xgboost_classifier(
        ...
        # Apply the model
        pred_prob_training_y = xgb_model.predict_proba(train_X)
        pred_prob_test_y = xgb_model.predict_proba(test_X)
        pred_training_y = xgb_model.predict(train_X)
        pred_test_y = xgb_model.predict(test_X)
        frame_prob_training_y = pd.DataFrame(
            pred_prob_training_y, columns=xgb_model.classes_
        )
        frame_prob_test_y = pd.DataFrame(pred_prob_test_y, columns=xgb_model.classes_)
    ...
    def logistic_regression(
        ...
        if gridsearch:
            ...
            logreg = GridSearchCV(
                LogisticRegression(), tuned_parameters, cv=3, scoring="roc_auc"
            )
        else:
        ...
        # Apply the model
        pred_prob_training_y = logreg.predict_proba(train_X)
        pred_prob_test_y = logreg.predict_proba(test_X)
        pred_training_y = logreg.predict(train_X)
        pred_test_y = logreg.predict(test_X)
        frame_prob_training_y = pd.DataFrame(
            pred_prob_training_y, columns=logreg.classes_
        )
        frame_prob_test_y = pd.DataFrame(pred_prob_test_y, columns=logreg.classes_)
